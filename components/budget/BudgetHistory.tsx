      ...
      {historyGroups.length === 0 ? (
        ...
        return (
          <Card key={group.periodId} className="overflow-hidden ring-1 ring-black/5 shadow-glass bg-white/80 backdrop-blur-xl">
            ...
            >
              <div className="p-6 transition-colors hover:bg-white/90">
                <div className="flex justify-between items-center mb-4">
                  ...
                  <div className="text-right">
                    <div className={`text-xl font-bold tracking-tight ${savings >= 0 ? 'text-money-safe' : 'text-money-neg'}`}>
              ...
              <div className="px-4 py-2 bg-slate-50/50 border-t border-slate-100 flex justify-center">
                {isExpanded ? (
            ...
            {isExpanded && (
              <div className="bg-slate-50/50 p-6 border-t border-slate-100 space-y-3">
                ...
                {group.snapshots.sort((a, b) => (b.limit - b.totalSpent) - (a.limit - a.totalSpent)).map(bucket => {
                  ...
                  return (
                    <div key={bucket.id} className="bg-white/80 backdrop-blur-sm p-4 rounded-xl ring-1 ring-black/5 shadow-sm">
                      <div className="flex justify-between items-center mb-2">
                        <span className="font-bold text-slate-700">{bucket.bucketName}</span>
                        <span className={`text-sm font-bold ${bucketSavings >= 0 ? 'text-money-safe' : 'text-money-neg'}`}>
                          ${bucket.totalSpent.toLocaleString()} <span className="text-slate-400 font-normal">/ ${bucket.limit.toLocaleString()}</span>
                        </span>
                      </div>
