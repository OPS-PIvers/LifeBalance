  ...
  return (
    <div
      ...
      className={`p-4 rounded-2xl border flex items-center justify-between transition-all duration-300 group cursor-pointer ${
        isSelected
          ? 'bg-slate-50 border-slate-300 ring-1 ring-slate-300 shadow-md'
          : 'bg-white/60 backdrop-blur-sm border-transparent ring-1 ring-black/5 hover:bg-white hover:shadow-glass hover:scale-[1.005]'
      }`}
    >
      <div className="flex items-center gap-4 overflow-hidden">
        ...
        <div className={`w-12 h-12 rounded-2xl flex items-center justify-center shrink-0 shadow-sm ${
           tx.category === INCOME_CATEGORY ? 'bg-emerald-50 text-emerald-600' : 'bg-white text-slate-400'
        }`}>
        ...
        <div className="min-w-0">
          <div className="flex items-center gap-2">
            <p className="font-semibold text-slate-900 truncate text-base tracking-tight">{tx.merchant}</p>
          ...
          <p className="text-xs text-slate-500 truncate flex items-center gap-1.5 mt-0.5">
            ...
            {tx.store && (
              <>
                <span className="w-1 h-1 rounded-full bg-slate-300" />
                <span className="font-medium text-slate-500">{tx.store}</span>
              </>
            )}
          </p>
        </div>
            ...
            {onMore && (
              <div className="flex sm:hidden">
                <Button
                  ...
                  className="text-slate-400 active:bg-slate-100 rounded-xl"
