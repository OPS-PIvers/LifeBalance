 ...
 *
 * The manage LIST is collapsed by default (via `CollapsibleSection`) — the same
 * rewards already appear in the store grid above, so showing them twice by
 * default was redundant; the count stays glanceable via the collapsed summary
 * and "Add reward" stays one tap away via the section's `action` slot. The
 * create/edit form itself lives in a `Drawer` bottom sheet rather than inline,
 * so opening it (e.g. via "Add reward") never requires expanding the list.
 *
  ...
  return (
    <>
      <CollapsibleSection
        ...
        summary={String(rewardsInventory.length)}
        defaultOpen={false}
        action={
          !formOpen ? (
            <button
