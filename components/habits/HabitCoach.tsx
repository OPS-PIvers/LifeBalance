...
import type { HabitPatternInsight } from '@/services/geminiService';
  ...
  if (habits.length === 0) {
    return (
      <div className="text-center py-12 px-4">
        <div className="w-16 h-16 bg-brand-100 text-brand-400 rounded-full flex items-center justify-center mx-auto mb-4">
          <Trophy size={32} />
        </div>
        <h3 className="text-lg font-bold text-brand-800 mb-2">No Habits Yet</h3>
        <p className="text-brand-400 text-sm">Start tracking some habits to unlock coaching insights!</p>
      </div>
    );
  }
      ...
      {!hasRun ? (
        ...
        <div className="space-y-4 animate-in fade-in slide-in-from-bottom-4">
          <div className="flex justify-between items-center px-2">
            <h3 className="text-sm font-bold text-brand-500 uppercase tracking-wider">Coach Insights</h3>
            <button
              onClick={handleAnalyze}
              disabled={loading}
              className="text-xs font-bold text-brand-600 flex items-center gap-1 hover:bg-brand-100 px-2 py-1 rounded-lg transition-colors"
            >
          ...
          <div className="grid gap-4">
            {insights.map((insight, idx) => (
              <div key={idx} className="bg-white p-5 rounded-2xl shadow-sm border border-brand-100 flex gap-4">
                <div className={`p-3 rounded-xl h-fit shrink-0 ${
                  insight.type === 'praise' ? 'bg-amber-100 text-amber-600' :
                  insight.type === 'critique' ? 'bg-rose-100 text-rose-600' :
                ...
                <div>
                  <h4 className="font-bold text-brand-800 mb-1">{insight.title}</h4>
                  <p className="text-sm text-brand-500 leading-relaxed">{insight.description}</p>
                  {insight.relatedHabitId && (
                     <div className="mt-2 inline-flex items-center gap-1.5 px-2 py-1 bg-brand-50 rounded-lg text-xs font-medium text-brand-400">
                        <TrendingUp size={12} />
                        Related Habit
                     </div>
                  )}
            ...
            {insights.length === 0 && !loading && (
                <div className="text-center p-8 bg-brand-50 rounded-2xl border border-dashed border-brand-200">
                    <p className="text-brand-400 font-medium">No clear patterns found yet. Keep tracking!</p>
                </div>
            )}
          </div>
        </div>
