...
const ShoppingSettingsModal: React.FC<Props> = ({ isOpen, onClose, initialTemplateData }) => {
  ...
  return (
    ...
    >
      {/* Tabs */}
      <div className="flex border-b border-slate-100 bg-white/80 backdrop-blur-xl sticky top-0 z-10">
          <button
        ...
        <div className="flex-1 overflow-y-auto p-6 sm:p-6 bg-slate-50/50">

          {activeTab === 'stores' && (
            <div className="space-y-6">
              {/* Add Store */}
              <div className="bg-white/80 backdrop-blur-xl p-5 rounded-2xl shadow-glass border border-slate-100">
                <h4 className="text-xs font-bold tracking-tight text-slate-400 mb-3 tracking-tight uppercase">Add New Store</h4>
                <form onSubmit={handleAddStore} className="space-y-4">
                  ...
                  <div className="flex gap-2">
                    <input
                      ...
                      className="flex-1 p-2.5 bg-slate-50 border border-slate-200/60 rounded-xl text-sm focus:ring-2 focus:ring-brand-500/50 focus:border-brand-500 outline-none"
                    />
                    <button
                      type="submit"
                      disabled={!newStoreName.trim()}
                      className="bg-slate-900 text-white px-5 py-2 rounded-xl font-bold tracking-tight hover:bg-slate-800 disabled:opacity-50 disabled:cursor-not-allowed transition-colors flex items-center gap-1 shadow-lg shadow-slate-200"
                    >
              ...
              <div className="space-y-3">
                <h4 className="text-xs font-bold tracking-tight text-slate-400 tracking-tight uppercase pl-1">My Stores</h4>
                {stores.length === 0 ? (
                  <p className="text-sm text-slate-400 italic pl-1">No stores added yet.</p>
                ) : (
                  <div className="grid gap-2">
                    {stores.map(store => (
                      <div key={store.id} className="bg-white/80 backdrop-blur-xl p-6 rounded-xl shadow-glass border border-slate-100 flex items-center justify-between group hover:shadow-soft transition-all">
                        {editingStoreId === store.id ? (
                           <div className="flex-1 space-y-2 mr-2">
                             ...
                             <div className="flex gap-2">
                               ...
                               <button onClick={() => setEditingStoreId(null)} className="text-slate-400 p-1 hover:bg-slate-100 rounded" aria-label="Cancel editing"><X className="w-4 h-4"/></button>
                             </div>
                           </div>
                        ) : (
          ...
          {activeTab === 'categories' && (
            <div className="space-y-6">
                <div className="bg-white/80 backdrop-blur-xl p-6 rounded-xl shadow-glass border border-slate-100">
                    <h4 className="text-sm font-bold tracking-tight text-slate-700 mb-3 tracking-tight uppercase">Add Category</h4>
                    <div className="flex gap-2">
                        <input
                            type="text"
                            ...
                            className="flex-1 p-2 border border-slate-300 rounded-lg text-sm focus:ring-2 focus:ring-brand-500 focus:border-brand-500 outline-none"
                        />
                        <button
                            onClick={addCategory}
                ...
                <div className="space-y-2">
                    <div className="flex items-center justify-between pl-1">
                         <h4 className="text-sm font-bold tracking-tight text-slate-500 tracking-tight uppercase">Active Categories</h4>
                         <button
                    ...
                    <div className="flex flex-wrap gap-2">
                        {localCategories.map(cat => (
                            <div key={cat} className="flex items-center gap-1 bg-white/80 backdrop-blur-xl border border-slate-200/60 pl-3 pr-1 py-1.5 rounded-full shadow-glass text-sm">
              ...
              {!editingTemplate ? (
                <>
                  <button
                    ...
                    className="w-full py-3 border-2 border-dashed border-slate-300 rounded-xl text-slate-500 font-medium hover:border-brand-300 hover:text-brand-600 hover:bg-brand-50 transition-all flex items-center justify-center gap-2"
                  >
                  ...
                  <div className="space-y-3">
                    {quickStockLists.map(list => {
                       ...
                       return (
                      <div key={list.id} className="bg-white/80 backdrop-blur-xl p-6 rounded-xl shadow-glass border border-slate-100 flex items-center justify-between group">
                        <div className="flex items-center gap-3">
                           <div className={`w-10 h-10 rounded-full flex items-center justify-center ${color.bg} ${color.text}`}>
                              <Icon className="w-5 h-5" />
                           </div>
                           <div>
                              <h4 className="font-bold tracking-tight text-slate-800">{list.name}</h4>
                ...
                <div className="space-y-4">
                  <div className="flex items-center justify-between">
                     <h4 className="font-bold tracking-tight text-slate-800">{editingTemplate.id ? 'Edit Template' : 'New Template'}</h4>
                     <button onClick={() => setEditingTemplate(null)} aria-label="Close"><X className="w-5 h-5 text-slate-400" /></button>
                  </div>

                  <div className="bg-white/80 backdrop-blur-xl p-6 rounded-xl shadow-glass border border-slate-100 space-y-3">
                    <div className="flex gap-2">
                      <input
                        type="text"
                        ...
                        className="flex-1 p-2 border border-slate-300 rounded-lg text-sm focus:ring-2 focus:ring-brand-500 outline-none"
                        autoFocus
                      />
                    </div>
                    ...
                    <div className="flex flex-col gap-3 p-3 bg-slate-50 rounded-xl border border-slate-100">
                         <div>
                            <span className="text-xs font-bold tracking-tight text-slate-400 uppercase mb-2 block">Icon</span>
                                    ...
                                    <button
                                        ...
                                        className={`p-2 rounded-lg transition-all shrink-0 ${
                                            (editingTemplate.icon || 'ShoppingBag') === id
                                                ...
                                                : 'bg-white/80 backdrop-blur-xl text-slate-400 hover:bg-slate-100 hover:text-slate-600'
                                        }`}
                                    >
                                        <Icon className="w-5 h-5" />
                                    </button>
                         ...
                         <div>
                            <span className="text-xs font-bold tracking-tight text-slate-400 uppercase mb-2 block">Color</span>
                            <div className="flex gap-2 overflow-x-auto p-2 scrollbar-hide">
                                {Object.values(STORE_COLORS).map((color) => (
                                    <button
                                        key={color.id}
                    ...
                    <div className="relative">
                      ...
                      <input
                        ...
                        className="w-full pl-9 p-2 bg-slate-50 border border-slate-200/60 rounded-lg text-sm focus:ring-2 focus:ring-brand-500 outline-none"
                      />
                    </div>
                      ...
                      .map(item => {
                        ...
                        return (
                          <button
                            ...
                            className={`w-full flex items-center justify-between p-2 rounded-lg text-sm transition-colors bg-white/80 backdrop-blur-xl border ${
                              isSelected
        ...
        {activeTab === 'categories' && (
            <div className="p-6 border-t border-slate-100 bg-white/80 backdrop-blur-xl">
                <button
                    ...
                    className="w-full py-3 bg-brand-800 text-white font-bold tracking-tight rounded-xl shadow-lg transition-all active:scale-95 disabled:opacity-50 disabled:active:scale-100"
                >
