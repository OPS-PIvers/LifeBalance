...
const ShoppingSettingsModal: React.FC<Props> = ({ isOpen, onClose, initialTemplateData }) => {
  ...
  return (
    ...
    >
      {/* Tabs */}
      <div className="px-4 py-2 bg-white sticky top-0 z-10 border-b border-slate-100/50">
          <div className="flex bg-slate-100 p-1 rounded-xl">
            <button
              onClick={() => setActiveTab('stores')}
              className={`flex-1 py-2 text-sm font-medium rounded-lg transition-all ${
                activeTab === 'stores' ? 'bg-white shadow-sm text-slate-900' : 'text-slate-500 hover:text-slate-700'
              }`}
            >
              Stores
            </button>
            <button
              ...
              className={`flex-1 py-2 text-sm font-medium rounded-lg transition-all ${
                activeTab === 'categories' ? 'bg-white shadow-sm text-slate-900' : 'text-slate-500 hover:text-slate-700'
              }`}
            >
              Categories
            </button>
            <button
              ...
              className={`flex-1 py-2 text-sm font-medium rounded-lg transition-all ${
                activeTab === 'templates' ? 'bg-white shadow-sm text-slate-900' : 'text-slate-500 hover:text-slate-700'
              }`}
            >
              Templates
            </button>
        ...
        <div className="flex-1 overflow-y-auto p-4 sm:p-6 bg-slate-50/50">
          ...
          {activeTab === 'stores' && (
            <div className="space-y-6">
              {/* Add Store */}
              <div className="bg-white/80 backdrop-blur-xl p-5 rounded-2xl shadow-glass ring-1 ring-black/5">
                ...
                <form onSubmit={handleAddStore} className="space-y-4">
                    ...
                    {Object.values(STORE_COLORS).map((color) => (
                      <button
                        ...
                        className={`w-7 h-7 rounded-full border-2 transition-all flex-shrink-0 ${color.bg} ${
                          newStoreColor === color.id ? 'border-slate-600 scale-110 ring-2 ring-slate-200 shadow-md' : 'border-transparent hover:scale-105 opacity-70 hover:opacity-100'
                        }`}
                        title={color.label}
                  ...
                  <div className="flex gap-2">
                    <input
                      ...
                      className="flex-1 p-3 bg-white/50 backdrop-blur-sm border border-slate-200/60 rounded-xl text-sm focus:ring-2 focus:ring-slate-400/20 focus:border-slate-400 outline-none text-slate-700 placeholder:text-slate-400 transition-all"
                    />
                    <button
                      type="submit"
              ...
              <div className="space-y-3">
                ...
                {stores.length === 0 ? (
                  ...
                  <div className="grid gap-2">
                    {stores.map(store => (
                      <div key={store.id} className="bg-white p-4 rounded-xl shadow-sm border border-slate-100/50 flex items-center justify-between group hover:shadow-md transition-all">
                           ...
                           <div className="flex-1 space-y-2 mr-2">
                                ...
                                {Object.values(STORE_COLORS).map((color) => (
                                  <button
                                    ...
                                    className={`w-6 h-6 rounded-full border-2 transition-all flex-shrink-0 ${color.bg} ${
                                      editStoreColor === color.id ? 'border-slate-600 scale-110 ring-1 ring-slate-200' : 'border-transparent hover:scale-105'
                                    }`}
                                    title={color.label}
                                    aria-label={`Select color ${color.label}`}
                                  />
                             ...
                             <div className="flex gap-2">
                               <input
                                  ...
                                  className="flex-1 p-2 bg-white border border-slate-200 rounded-lg text-sm outline-none focus:ring-2 focus:ring-slate-400/20"
                               />
                               <button onClick={handleUpdateStore} className="text-emerald-600 p-2 hover:bg-emerald-50 rounded-lg" aria-label="Save store name"><Save className="w-4 h-4"/></button>
                            ...
                            <div className="flex items-center gap-3">
                                ...
                                <span className="font-semibold text-slate-800 tracking-tight">{store.name}</span>
                            </div>
                        )}
          ...
          {activeTab === 'categories' && (
            <div className="space-y-6">
                <div className="bg-white/80 backdrop-blur-xl p-6 rounded-2xl shadow-glass border border-slate-100/50 ring-1 ring-black/5">
                    ...
                    <div className="flex gap-2">
                        <input
                            ...
                            className="flex-1 p-3 bg-white/50 backdrop-blur-sm border border-slate-200/60 rounded-xl text-sm focus:ring-2 focus:ring-slate-400/20 focus:border-slate-400 outline-none text-slate-700"
                        />
                        <button
                            onClick={addCategory}
                            disabled={!newCategoryName.trim()}
                            className="bg-slate-900 text-white px-5 py-2 rounded-xl font-bold hover:bg-slate-800 disabled:opacity-50 disabled:cursor-not-allowed transition-colors shadow-lg shadow-slate-200"
                        >
                            <Plus className="w-4 h-4" />
                        </button>
                    </div>
                ...
                <div className="space-y-2">
                    ...
                    <div className="flex flex-wrap gap-2">
                        {localCategories.map(cat => (
                            <div key={cat} className="flex items-center gap-1 bg-white border border-slate-200 pl-3 pr-1 py-1.5 rounded-full shadow-sm text-sm hover:shadow-md transition-all">
              ...
              {!editingTemplate ? (
                <>
                  <button
                    ...
                    className="w-full py-4 border-2 border-dashed border-slate-200 rounded-2xl text-slate-500 font-medium hover:border-slate-400 hover:text-slate-700 hover:bg-slate-50 transition-all flex items-center justify-center gap-2 group"
                  >
                  ...
                  <div className="space-y-3">
                    {quickStockLists.map(list => {
                       ...
                       return (
                      <div key={list.id} className="bg-white p-4 rounded-xl shadow-sm border border-slate-100/50 flex items-center justify-between group hover:shadow-md transition-all">
                        <div className="flex items-center gap-3">
                           <div className={`w-12 h-12 rounded-2xl flex items-center justify-center shadow-sm ${color.bg} ${color.text}`}>
                              <Icon className="w-6 h-6" />
                           </div>
                           <div>
                              <h4 className="font-bold text-slate-800 tracking-tight">{list.name}</h4>
                    ...
                    {quickStockLists.length === 0 && (
                      <p className="text-center text-slate-400 text-sm py-8 italic">No templates yet. Create one for &quot;Work Week&quot;, &quot;Camping&quot;, etc.</p>
                    )}
                  </div>
                </>
              ) : (
                <div className="space-y-4">
                  <div className="flex items-center justify-between">
                     ...
                     <button onClick={() => setEditingTemplate(null)} aria-label="Close" className="p-2 hover:bg-slate-100 rounded-full"><X className="w-5 h-5 text-slate-500" /></button>
                  </div>

                  <div className="bg-white/80 backdrop-blur-xl p-5 rounded-2xl shadow-glass border border-slate-100/50 space-y-4 ring-1 ring-black/5">
                    <div className="flex gap-2">
                      <input
                        type="text"
                        ...
                        className="flex-1 p-3 bg-white/50 backdrop-blur-sm border border-slate-200/60 rounded-xl text-sm focus:ring-2 focus:ring-slate-400/20 focus:border-slate-400 outline-none text-slate-700 placeholder:text-slate-400"
                        autoFocus
                      />
                    </div>

                    {/* Icon & Color Selection */}
                    <div className="flex flex-col gap-3 p-4 bg-slate-50/50 rounded-xl border border-slate-100/50">
                         <div>
                    ...
                    <div className="relative group">
                      ...
                      <input
                        ...
                        className="w-full pl-9 p-3 bg-white/50 backdrop-blur-sm border border-slate-200/60 rounded-xl text-sm focus:ring-2 focus:ring-slate-400/20 focus:border-slate-400 outline-none text-slate-700 placeholder:text-slate-400"
                      />
                    </div>
                    ...
                    {itemSearch.trim() && !groceryCatalog.some(i => i.name.toLowerCase() === itemSearch.trim().toLowerCase()) && (
                        <button
                            ...
                            className="w-full flex items-center gap-2 p-3 bg-white border border-slate-200 border-dashed rounded-xl text-sm font-medium text-slate-600 hover:text-slate-900 hover:bg-slate-50 transition-colors"
                        >
                            <Plus className="w-4 h-4" />
                        ...
                        return (
                          <button
                            ...
                            className={`w-full flex items-center justify-between p-3 rounded-xl text-sm transition-all ${
                              isSelected
                                ? 'bg-emerald-50 border-emerald-100 text-emerald-800 font-medium border'
        ...
        {activeTab === 'categories' && (
            <div className="p-4 border-t border-slate-100/50 bg-white">
                <button
                    ...
                    className="w-full py-3 bg-slate-900 text-white font-bold rounded-xl shadow-lg transition-all active:scale-95 disabled:opacity-50 disabled:active:scale-100 hover:bg-slate-800"
                >
