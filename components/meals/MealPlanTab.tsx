...
const MealPlanTab: React.FC = () => {
  ...
  const [actionMealPlanItem, setActionMealPlanItem] = useState<{ item: MealPlanItem, meal?: Meal } | null>(null);
  ...
  return (
    <div className="space-y-6 pb-20">
                            ...
                            {planItems.length > 0 ? planItems.map((planItem) => {
                                ...
                                return (
                                    <div key={planItem.id} className="group bg-white/60 border border-slate-200/60 rounded-xl p-4 shadow-sm hover:shadow-md hover:bg-white transition-all flex justify-between items-start gap-4">
                                        ...
                                        <button
                                            onClick={() => setActionMealPlanItem({ item: planItem, meal: linkedMeal ?? undefined })}
                                            className="sm:hidden p-2 text-gray-400 hover:text-gray-600 active:scale-95 transition-transform"
      ...
      <Drawer
        isOpen={!!actionMealPlanItem}
        onClose={() => setActionMealPlanItem(null)}
        title="Meal Options"
      >
            ...
            {actionMealPlanItem && (
                <>
                  <Button
                    ...
                    onClick={() => {
                        handleEditMealPlanItem(actionMealPlanItem.item, actionMealPlanItem.meal);
                        setActionMealPlanItem(null);
                    }}
                  >
                    Edit Meal
                  </Button>
                  ...
                  <Button
                    ...
                    leftIcon={<Trash2 />}
                    onClick={() => {
                        deleteMealPlanItem(actionMealPlanItem.item.id);
                        setActionMealPlanItem(null);
                    }}
                  >
                    Delete from Plan
