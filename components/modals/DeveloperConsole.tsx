    ...
    <Modal isOpen={isOpen} onClose={onClose} ariaLabelledBy="dev-console-title" maxWidth="max-w-4xl">
      <div className="flex flex-col h-[70vh]">
        <div className="p-6 border-b border-slate-200/60 flex justify-between items-center">
            <h2 id="dev-console-title" className="text-xl font-bold tracking-tight">Developer Console</h2>
            <button
              onClick={onClose}
        ...
        {/* Tabs */}
        <div className="flex border-b border-slate-200/60 bg-slate-50">
          <button
            onClick={() => setActiveTab('testers')}
            className={`px-4 py-3 text-sm font-medium border-b-2 transition-colors ${
              activeTab === 'testers' ? 'border-brand-600 text-brand-600 bg-white/80 backdrop-blur-xl' : 'border-transparent text-slate-500 hover:text-slate-700'
            }`}
          >
            Beta Testers
          </button>
          <button
            ...
            className={`px-4 py-3 text-sm font-medium border-b-2 transition-colors ${
              activeTab === 'ai_meter' ? 'border-brand-600 text-brand-600 bg-white/80 backdrop-blur-xl' : 'border-transparent text-slate-500 hover:text-slate-700'
            }`}
          >
            AI Usage Meter
          </button>
          <button
            ...
            className={`px-4 py-3 text-sm font-medium border-b-2 transition-colors ${
              activeTab === 'reports' ? 'border-brand-600 text-brand-600 bg-white/80 backdrop-blur-xl' : 'border-transparent text-slate-500 hover:text-slate-700'
            }`}
          >
            Feedback Reports
          </button>
        ...
        {/* Content */}
        <div className="flex-1 overflow-y-auto p-6 bg-white/80 backdrop-blur-xl">
          {loading ? (
            ...
            <>
              {activeTab === 'testers' && (
                <div className="space-y-6">
                  <div className="flex gap-2 p-6 bg-slate-50 rounded-xl border border-slate-200/60">
                    <input
                      type="email"
                        ...
                        {testers.map(t => (
                          <tr key={t.id} className="hover:bg-slate-50">
                            ...
                            <td className="p-3 flex gap-2">
                              <button onClick={() => toggleTesterStatus(t.id, t.status)} className="text-blue-600 hover:underline text-xs font-bold tracking-tight">
                ...
                <div className="space-y-4">
                  <div className="grid grid-cols-2 gap-6">
                    <div className="bg-blue-50 p-6 rounded-xl border border-blue-100">
                      ...
                      <p className="text-3xl font-bold tracking-tight text-blue-900">{households.length}</p>
                    </div>
                  </div>

                  <div className="border rounded-xl overflow-hidden">
                    <div className="bg-slate-50 px-4 py-3 border-b font-medium text-slate-700">Household Usage (Daily)</div>
                    <div className="divide-y">
                        {households.map(h => {
                            const usage = h.aiUsage?.dailyCount || 0;
                            const percentage = Math.min((usage / 20) * 100, 100);
                            return (
                                <div key={h.id} className="p-6 flex items-center justify-between hover:bg-slate-50">
                                    <div>
                                        <p className="font-bold tracking-tight text-slate-800">{h.name}</p>
                                    ...
                                    <div className="flex items-center gap-6 w-1/2">
                                        ...
                                        <span className={`text-sm font-mono font-bold tracking-tight w-12 text-right ${percentage > 90 ? 'text-red-600' : 'text-slate-600'}`}>
                                            {usage}/20
                                        </span>
                                    </div>
                                </div>
              ...
              {activeTab === 'reports' && (
                <div className="space-y-4">
                    {reports.map(report => (
                        <div key={report.id} className="p-6 rounded-xl border border-slate-200/60 hover:shadow-soft transition-shadow">
                            <div className="flex justify-between items-start mb-2">
                                <div>
                                    <span className="text-xs font-mono bg-slate-100 px-2 py-1 rounded text-slate-600 font-bold tracking-tight">{report.version}</span>
                            ...
                            <div className="pt-3 border-t border-slate-50 grid grid-cols-1 md:grid-cols-2 gap-2 text-xs text-slate-500 font-mono">
