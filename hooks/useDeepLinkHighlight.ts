...
/**
 * How long a deep-link highlight stays lit. Exported because the legacy
 * `?todo=` path in `ToDosPage` runs its own timer and must fade on the same
 * schedule — a second hard-coded 2200 there would silently diverge the moment
 * this one changed.
 */
export const HIGHLIGHT_DURATION_MS = 2200;
 ...
 */
export function useDeepLinkHighlight(durationMs: number = HIGHLIGHT_DURATION_MS): string | null {
