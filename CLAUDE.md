...
**A streak period must be COMPLETED, not merely touched.** A period counts toward a streak only once its `targetCount` was actually met — the habit-level walk gets this for free (`completedDates` only gains a date when the target is crossed), and the per-member walk gets it from `memberStreakDates` ([utils/habitAttribution.ts](utils/habitAttribution.ts)), which filters the member's completion dates through the same `periodCompleted` gate `memberPointsForHabitOnDate` uses to decide whether to pay them at all. Before that filter, a weekly `targetCount: 3` habit logged once a week built a 2-week member "streak" paying 2× while the habit's own `streakDays` correctly read 0. **The gate is the PERIOD, not the member's own units against the target** — contribute to a period that got finished and it counts for you, so two members who finish a `targetCount: 2` habit together every day both keep their streaks; requiring a solo fill is a third question nothing else asks and would contradict the award being paid. At `targetCount <= 1` the filter is skipped entirely (a unit and a completion are the same event there), so nothing else moves.
