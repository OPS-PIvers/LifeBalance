...
## 2026-02-28 - Standardized Button Shadows
**Drift:** The `Button` component used hardcoded shadow values for `primary`, `success`, `warning`, and `destructive` variants, creating "Snowflakes" and making it difficult to maintain a consistent elevation system.
**Fix:** Added semantic shadow tokens (`btn-primary`, `btn-primary-hover`, `btn-secondary`, `btn-success`, `btn-warning`, `btn-destructive`) to `tailwind.config.js` and refactored `components/ui/Button.tsx` to use these tokens.
