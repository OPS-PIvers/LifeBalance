...
| 090 | Modular pages — per-household page/tab on-off toggles | — (new UX track) | C | LOW | 🚧 **IN PROGRESS** ([090](./090-modular-pages.md)) — design grilled + locked (8 decisions); 4 PRs (PR1 core: schema + `moduleVisibility` helper/hook + Settings "App Modules" UI + dynamic footer + route guards + Plan-tab fallback). **No rules change; ships live, default all-on** (zero change for existing households) |
