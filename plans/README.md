> **Historical — superseded by [`advisor-plans/`](../advisor-plans/README.md) (2026-07-04) for current priorities.** Kept as the record of the 2026-06 planning pass and what shipped from it.
...
| 016 | Safe-to-Spend **pool + bucket-tracking breakdown** — fix bill↔bucket vanishing bug (all unpaid bills subtract) + new StS breakdown drawer (per-bucket remaining + leftover) | — | C | LOW-MED | 📝 **PLAN WRITTEN** ([016](./016-safe-to-spend-bucket-breakdown.md), against `388fd11`) — from a design interview; **pool + tracking-overlay model (NOT envelope — buckets never reduce StS)**. No rules/migration; one live-number change (bucketed bills stop vanishing). Deletes `resolveBucketForCalendarItem`; paid bills → `BUDGETED_IN_CALENDAR` |
...
| 050–052 | Stripe billing code + entitlements + freemium gating | 2 | C→H | MED (HIGH for 051 rules) | 📝 **PLAN WRITTEN** ([050](./050-stripe-monetization.md) · [051](./051-server-side-entitlement-limits.md)) — greenfield, additive (per-household model already fits). Build dormant against test keys; entitlements server-truth; 051 rules change behind Plan 010 tests + human-watch. **051 (server-side count-limit enforcement) is a hard prerequisite for flipping `billingEnabled`** — member cap in rules (grandfathered) + kid cap via new `createkidprofile` callable; `historyMonths` deferred. **Human: Stripe account + entity + bank + live keys/webhook secret** — turnkey [`STRIPE_SETUP_RUNBOOK`](../docs/STRIPE_SETUP_RUNBOOK.md) (Phase 0 startable today). Don't start before Phase 0/1 exit |
...
| 090 | Modular pages — per-household page/tab on-off toggles | — (new UX track) | C | LOW | ✅ **DONE** ([090](./090-modular-pages.md)) — 4 PRs merged + deployed ([#720](https://github.com/OPS-PIvers/LifeBalance/pull/720) core · [#721](https://github.com/OPS-PIvers/LifeBalance/pull/721) capture · [#722](https://github.com/OPS-PIvers/LifeBalance/pull/722) dashboard/toolbar · [#723](https://github.com/OPS-PIvers/LifeBalance/pull/723) graceful degradation). `Household.moduleVisibility` + `utils/moduleVisibility` cascade through nav/routes/capture/toolbar/widgets; Settings → "App Modules". **No rules change; default all-on** (zero change for existing households) |
