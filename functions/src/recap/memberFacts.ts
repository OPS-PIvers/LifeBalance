 ...
 * week built a 2-week "streak" paying 2x, while the habit's own `streakDays`
...
function memberTopStreak(
  ...
  for (const habit of habits) {
    // Completed periods only -- a "top streak" tile must never crown a habit
    // the member has not once finished (see memberStreakDatesFor).
